...
- (void)routine
{
    if ( self.sending )
	{
        ...
        self.HTTP_POST(requestURI).HEADER(@"ContentType", @"application/x-www-form-urlencoded; charset=utf-8").PARAM(@"message", contentbase64).PARAM(@"aid", self.aid).PARAM(@"uid",self.uid).PARAM(@"ip", self.ip).PARAM(@"cid", self.op);
	}
