...
//        requestURI=[requestURI stringByAppendingFormat:@"&fid=%@&uid=%@&%@=%@",self.fid,self.uid,self.filename,self.filedata];
        ...
        self.HTTP_POST(requestURI).PARAM([self.req objectToDictionary]).HEADER(@"enctype", @"multipart/form-data; boundary=AaB03x").PARAM(@"uid",self.uid).PARAM(@"fid",self.fid).FILE(@"files",self.filedata).TIMEOUT(180);
