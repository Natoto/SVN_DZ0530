...
DEF_SIGNAL(WILL_ACTIVE)		// 将要获取焦点
DEF_SIGNAL(WILL_DEACTIVED)		// 已经获取焦点)
...
ON_SIGNAL3(ForumlistModel, RELOADED, signal)
{
    ...
    if ([[achile class] isSubclassOfClass:[forums class]] && achile.child.count) {
        self.subchildAry = [self readsubchildAry:achile.child parentforum:achile];
        ...
        case 0:
        {
            ...
            if ([[self.locate.child.child class] isSubclassOfClass:[NSArray class]] && self.locate.child.child.count) {
                if ([[self.locate.child class]  isSubclassOfClass:[self.locate.child class]] && self.locate.child.child.count) {
                    self.subchildAry = [self readsubchildAry:self.locate.child.child parentforum:self.locate.child];// self.locate.child.child;
...
-(NSArray *)readsubchildAry:(NSArray *)dic parentforum:(forums *)parentforum
{
    ...
    forums *aforums = [[forums alloc] init];
    aforums.fid = parentforum.fid;
    aforums.name = @"不选";
    [array addObject:aforums];
    return array;
}
...
//    [self sendUISignal:self.WILL_DEACTIVED];
    CATransition *animation = [CATransition  animation];
    animation.delegate = self;
    animation.duration = kDuration;
    animation.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut];
...
{
    UIView * view = [superView viewWithTag:self.tag];
    return view?YES:NO;
}
...
//    [self sendUISignal:self.WILL_ACTIVE];
    CATransition *animation = [CATransition  animation];
    animation.delegate = self;
    animation.duration = kDuration;
    animation.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut];
