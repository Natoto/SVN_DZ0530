...
@interface D0_Mine_iphone ()<UITableViewDataSource,UITableViewDelegate,D0_CheckinsViewDelegate,DZ_PopupBoxDelegate,DZ_TimerDelegate>
{
    NSArray *Toolsarray;
    NSArray *ToolsIcon;
...
@property(nonatomic,retain)DZ_Timer        * refreshtimer;
@end
...
//    [self.refreshtimer invalidateDZ_RemindTime];
...
//    [self.refreshtimer invalidateDZ_RemindTime];
    [self.refreshtimer freshNewRemindMessage:REFLASHTIME fire:NO astepblock:^(BOOL result) {}];
...
ON_DID_DISAPPEAR(signal)
{
    ...
    BeeLog(@"self.navigationController.childViewControllers.count = %d",self.navigationController.childViewControllers.count);
    
    if (self.navigationController.childViewControllers.count == 1 && [SettingModel sharedInstance].autoRefreshmsgmode)//进入到子层不刷新消息
    {
        BeeLog(@"--------开始自动刷新消息--------------");
//        [self.refreshtimer endRemind];
        [self.refreshtimer freshNewRemindMessage:[SettingModel sharedInstance].autoRefreshmsgmode fire:YES astepblock:^(BOOL result) {
            if (result) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row == 0) {
        ...
        if (self.lginModel.session) {
            ...
            NSInteger othernewcount = [strangerpmmodel sharedInstance].newstrangermsDic.allKeys.count + FRIENDSMESSAGE + SYSTEMMESSAGE + ACTVMESSAGE + THREADMESSAGE;
...
static int newmessagecount;
ON_SIGNAL3(RemindModel, RELOADED, signal)
{
//    if(self.remindModel.dialog.count)
    {
        NSInteger  newcount = self.remindModel.dialog.count +  FRIENDSMESSAGE +  SYSTEMMESSAGE + INNERMESSAGE +
        ACTVMESSAGE + THREADMESSAGE;
        newmessagecount = newmessagecount + newcount ;
        if (newcount && newmessagecount && !self.isTopViewController) {
            [KGStatusBar showWithStatus:[NSString stringWithFormat:@"您有%d条新消息,请在我的中心查看！",newmessagecount]];
...
ON_SIGNAL3(Allpm_StrangerModel, RELOADED, signal)
{
     ...
     NSInteger  count = self.remindModel.dialog.count + self.remindModel.sysautomatic.count + self.remindModel.friendsautomatic.count + strangerpmmodel.strangermsDic.allKeys.count + self.remindModel.threadautomatic.count;
    [UserModel sharedInstance].messageCount = [NSString stringWithFormat:@"%ld",(long)count];

    [self.list reloadData];
}

//-(void)dismissKGStatusBar
//{
//    [KGStatusBar dismiss];
//}
...
ON_NOTIFICATION3(BeeReachability, CHANGED, notify)
{
    ...
    {
        [KGStatusBar showSuccessWithStatus:@"2G/3G/4G状态下关闭自动刷新消息!"];
        [SettingModel sharedInstance].autoRefreshmsgmode = REFRESHMSG_NO;
    }
}
