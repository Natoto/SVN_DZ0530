...
//        } else if ([setting.mode isEqualToString:@"3"]) {
//            bee.ui.router[self.TAB_HOME] = [A0_HomePage3_iPhone class];
//        } else {
...
//        [self.tabbar selectHomepage];
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            //设置首页模版
            [Switch_OnoffModel getSwitch_OnOff:^(onoff *reciveData) {
                BeeLog(@"获得开关信息 %@ ",reciveData);
                if (reciveData.isportal) {
                    bee.ui.router[self.TAB_HOME] = [A0_HomePage3_iPhone class];
                } else {
                    DZ_SystemSetting *setting = [[DZ_SystemSetting alloc] init];
                    if ([setting.mode isEqualToString:@"2"]) {
