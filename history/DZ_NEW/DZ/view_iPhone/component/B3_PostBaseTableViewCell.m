...
{
    //    SuppressPerformSelectorLeakWarning([self performSelector:loadcontentselector withObject:self.contentsAry withObject:[NSNumber numberWithBool:YES]]);
}
...
{
    if ((index +1) >= contents.count) return NO;

    content *acont=[self.contentsAry objectAtIndex:index + 1];

    if (acont.type.integerValue == contenttype ) return YES;
...
//|| acont.type.intValue == CONTENTTYPE_ABSTRACT

- (NSString *)loadAbstract:(content *)acont index:(int)index OriginY:(float )OriginY subContentAry:(NSMutableArray *)subContentAry contents:(NSArray *)contents
{
    ...
    NSString *rtlabelText=[[NSString alloc] initWithFormat:@""];
    if (acont.type.intValue == CONTENTTYPE_ABSTRACT) {
        rtlabelText=[rtlabelText stringByAppendingString:acont.msg];
        [subContentAry addObject:rtLabel];
        //摘要颜色为黄色
        rtLabel.textColor = [UIColor orangeColor];
    }
...
- (void)loadselfdefinecontents:(NSArray *)contents senddelegate:(BOOL)send
{
    ...
    for (int index=0; index<contents.count; index++)
    {
        ...
        else if(acont.type.intValue == CONTENTTYPE_ABSTRACT)//加摘要
        {
            NSString *resstring = [self loadAbstract:acont index:index OriginY:OriginY subContentAry:subContentAry contents:contents];
            NSArray *compants = [resstring componentsSeparatedByString:@":"];
            if (compants.count == 2) {
                NSString *compon1 = [compants objectAtIndex:0];
                NSString *compon2 = [compants objectAtIndex:1];
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(typeOfcell:)]) {
        CELL_TYPE topicart = [self.delegate  typeOfcell:self];
        if (topicart == CELL_MAINTOPIC) {
...
//    tempfontsize = tempfontsize * 2 + FONT_MIDDLEBASE;
    ...
    for (NSUInteger index=0; index<contents.count; index++)
    {
        ...
        else if(acont.type.intValue ==  CONTENTTYPE_ABSTRACT)
        {
            RCLabel *rtLabel = [[RCLabel alloc] initWithFrame:CGRectMake(MARGIN_LEFT, OriginY, CGRectGetWidth([UIScreen mainScreen].bounds) - MARGIN_RIGHT - MARGIN_LEFT, 30)];
            [rtLabel setFont:[UIFont systemFontOfSize:tempfontsize]];
            NSString *rtlabelText=[[NSString alloc] initWithFormat:@""];
...
- (void)reloadsubviews
{
    if (!self.delegate || ![self.delegate respondsToSelector:@selector(cellpost:)]) return;
    ...
    if (self.cellpost) {
        ...
        //点赞
        if (self.cellpost.support) _lblsupport.text = [NSString stringWithFormat:@"%@", self.cellpost.support];

        //点赞状态
        if (self.cellpost.support != nil) self.status = self.cellpost.status;
...
- (void)loadheaderviews
{
    ...
    // Initialization code
    if (!self.delegate || ![self.delegate respondsToSelector:@selector(frameOfCellHeader:)]) return;
...
- (void)headerviewTap
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(B3_PostBaseTableViewCell:tappedheaderview:)]) {
        ...
        if (headerviewSelected) headerviewSelected = NO;
        else headerviewSelected = YES;
...
- (void)setStatus:(NSNumber *)status
{
    ...
    if (cell_type == CELL_MAINTOPIC) {
        if (!_status.integerValue) [_btnsupport setImage:weidingtieimage02 forState:UIControlStateNormal];
        else [_btnsupport setImage:weidingtieimage01 forState:UIControlStateNormal];
    } else if (cell_type == CELL_REPLYTOPIC) {
        if (!_status.integerValue) [_btnsupport setImage:dingtie forState:UIControlStateNormal];
        else [_btnsupport setImage:weidingtie forState:UIControlStateNormal];
    }
    else if (cell_type == CELL_PORTALPOST) {
        if (!_status.integerValue) [_btnsupport setImage:dingtie forState:UIControlStateNormal];
        else [_btnsupport setImage:weidingtie forState:UIControlStateNormal];
    }
}
