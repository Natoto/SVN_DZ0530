...
- (void)loadTabelView
{
    ...
    tv = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, rect.size.width, rect.size.height - bee.ui.tabbar.frame.size.height)];
    tv.dataSource = self;
    tv.delegate = self;
    tv.tableHeaderView = [self loadCarouselViewWithImageArray:nil titleArray:nil];
...
//加载轮播图
- (UIView *)loadCarouselViewWithImageArray:(NSArray *)imageArray titleArray:(NSArray *)titleArray
{
    //视图数组
    if (!viewsArray) {
    ...
    //设置轮播图的文本
    [carouselView textLabelAtIndexUsingBlock:^(PFCarouselView *carouselView, UILabel *textLabel, NSInteger index) {
...
- (void)loadSlideData
{
    ...
    [self loadCarouselViewWithImageArray:imgArr titleArray:subArr];
}
...
//     if ([self.articleType isEqualToNumber:@1])
    if (!type | [type isEqualToNumber:@1] ) {
    ...
    {
        B3_PortalViewController *board = [[B3_PortalViewController alloc] init];
        board.tid = tid;
        board.articleType = type;
        [self.navigationController pushViewController:board animated:YES];
    }
