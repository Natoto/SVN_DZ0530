...
- (void)loadCarouselViewWithImageArray:(NSArray *)imageArray subjectArray:(NSArray *)subjectArray
{
    ...
    //添加视图
    [carouselView contentViewAtIndexUsingBlock:^UIView *(PFCarouselView *automaticScrollView, NSInteger index) {
        return viewsArray[index];
    }];

    //设置文本
    [carouselView textLabelAtIndexUsingBlock:^(PFCarouselView *automaticScrollView, UILabel *textLabel, NSInteger index) {
        textLabel.text = subjectArray[index];
    }];

    //视图数量
    [carouselView numberOfPagesInCarouselViewUsingBlock:^NSInteger(PFCarouselView *automaticScrollView) {
...
- (void)loadTabView
{
    if (!tabView) {
        ...
        tabView.heightOfItem = 30;

        tabView.itemNormalColor = [PFTabView colorFromHexRGB:@"868686"];
        UIColor *color = [DZ_SystemSetting sharedInstance].navigationBarColor;
        tabView.itemSelectedColor = color;
    ...
    //视图数目
    [tabView numberOfItemInTabViewUsingBlock:^NSUInteger(PFTabView *tabView) {
        return 4;
    }];

    //设置视图
    [tabView viewControllerOfItemAtIndexUsingBlock:^UIViewController *(PFTabView *tabView, NSUInteger index) {
        if (index == 3) {
    ...
    //设置标签尺寸
    [tabView textSizeOfItemInTabViewUsingBlock:^CGSize(PFTabView *tabView) {
        return CGSizeMake((self.view.frame.size.width - 40) / 4, 30.0f);
    }];

    //点击事件
    [tabView didSelectItemAtIndexUsingBlock:^(PFTabView *tabView, NSUInteger index) {
        if (index == 0) {
