...
//*/
            [cell tableViewCellLoadedUsingBlock:^(NSIndexPath *indexPath) {
                if (indexPath.row == 0) {
                    if (!imageLoaded) {
                        imageLoaded = YES;
                        [self.tableViewList reloadData];
...
///*
        ...
        if (indexPath.row == 0) {//内容页，头像，标题，摘要，层，用户名，发布日期
            ...
            B3_PostView *b3PostView = [[B3_PostView alloc] init];
            [b3PostView loadContents:self.articleModel.mainArcticle.content height:[b3PostView heightOfContents:self.articleModel.mainArcticle.content cellType:cellTypeArticla otherViewHeight:cell.secondTextLabel.frame.size.height + 100 + 20 indexPath:indexPath] contentView:cell.firstContentView];
            [cell.firstContentView addSubview:b3PostView];
...
//            [self.b3PostModel setupRowInTableViewCell:cell articleModel:self.articleModel viewControl:self indexPath:indexPath];
            ...
            cell.thirdTextLabel.text = [NSString stringWithFormat:@"%d楼:%@", position + 1, commentlist1.username];
        ...
        [postModel longPressUsingBlock:^(B3_PostModel *postModel, UIView *view, UIGestureRecognizer *recognizer) {
            //双击结束 和长按开始状态
