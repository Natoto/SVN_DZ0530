...
#import "Constants.h"
#import "B3_PortalViewController.h"
...
@interface B3_PortalViewController () <FaceBoardDelegate, MaskViewDelegate, D2_ChatInputViewDelegate, B3_Post_IWantApplyDelegate, UIAlertViewDelegate>
{
    float               historyY;
    BOOL                replyMode;
...
@implementation B3_PortalViewController
...
ON_NOTIFICATION3(B3_PostMenuView, delcollection, signal)
{
    ...
    if (!username) {
        ...
        if (self.collectModel.favid != 0) {
            ...
            self.delcollectionModel.favid = (NSNumber *)self.articleModel.mainArcticle.favid;
        }
...
//}
#pragma mark - article reply model
...
#pragma mark - deal activitymodel
...
- (void)viewDidLoad
{
    ...
    menuView = [[B3_PostMenuView alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width - 70, 20, 60, 100) itemNumber:[self.articleType isEqual:@1] ? 6 : 3];
    
    _cellsHeightDic = [[NSMutableDictionary alloc] initWithCapacity:0];
    for (int i = 0; i < self.articleModel.shots.count; i++) {
        [_cellsHeightDic setObject:[NSNumber numberWithFloat:60] forKey:[NSString stringWithFormat:@"%d",i]];
    }
    //回复区
...
- (void)showPreviewsBigImgview:(NSString *)url imageView:(BeeUIImageView *)imageView mainTopic:(BOOL)maintopic
{
    if (imageView.command ==COMMAND_URL && url && [url rangeOfString:@"file://"].location==NSNotFound) {
        ...
        if (maintopic || currentindexPath.integerValue < 0) {
            contentArray = [self articlecontent2contentAry:self.articleModel.mainArcticle.content];
        } else {
//            post *apost=
            commentlist * acommit =[self.articleModel.shots objectAtIndex:currentindexPath.integerValue];
            post *apost = [self commentlist2post:acommit];
...
- (void)applyActivity
{
    ...
    self.dealactivityModel.authorid = self.articleModel.mainArcticle.uid;
    self.dealactivityModel.message = [_applyActivityView.iWantapplyDic valueForKey:@"leavemessage"];
    self.dealactivityModel.subject =  self.articleModel.mainArcticle.title ;
...
-(content *)article_contentToconent:(article_content *)art_content
{
    content * acontent = [[content alloc] init];
    acontent.msg =   art_content.msg;
    acontent.type =  art_content.type;
    return acontent;
}
/*
...
-(post *)commentlist2post:(commentlist *)acoment
{
    post * apost = [[post alloc] init];
    apost.authorid = acoment.uid;
    apost.authorname = acoment.username;
    apost.usericon = acoment.avatar;
    apost.postsdate = acoment.dateline;
    apost.pid = acoment.cid;
//    apost.subtract = acoment.yinreply;
    content * aconte = [[content alloc] init];
    aconte.type = CONTENTTYPE_TEXT;
    aconte.msg = acoment.message;
    
    if (acoment.yinreply.length) {
        content * aconte2 = [[content alloc] init];
        aconte2.type = [NSNumber numberWithInt:CONTENTTYPE_QUOTEREPLY];
        aconte2.msg = acoment.yinreply;
        NSArray * contentary = [NSArray arrayWithObjects:aconte2,aconte, nil];
        apost.content = contentary;
        apost.position = acoment.position;
    }
    else
    {
        NSArray * contentary = [NSArray arrayWithObjects:aconte, nil];
        apost.content = contentary;
        apost.position = acoment.position;
    }
...
-(NSArray *)articlecontent2contentAry:(NSArray *)articleary
{
    NSMutableArray * contentarray = [[NSMutableArray alloc] initWithCapacity:0];
    for (NSInteger index = 0; index < articleary.count; index ++) {
        article_content * art_cont = [articleary objectAtIndex:index];
        content * cont = [self article_contentToconent:art_cont];
        [contentarray addObject:cont];
    }
    if(articleary.count)
    {
        article_content * art_cont = [articleary objectAtIndex:0];
        content * cont = [self article_contentToconent:art_cont];
        if (cont.msg.length > 80) {
        cont.msg = [NSString stringWithFormat:@"摘要: %@\n\n",[cont.msg substringToIndex:80]];
        }
        else
        {
            cont.msg = [NSString stringWithFormat:@"摘要: %@\n\n",cont.msg];
        }
        cont.type = [NSNumber numberWithInteger:CONTENTTYPE_ABSTRACT];
        [contentarray insertObject:cont atIndex:0];
    }
    return contentarray;
}


-(topic *)mainarticleTotopic:(portal_article *)article
{
    topic * atopic = [[topic alloc] init];
    atopic.authorid = article.uid;
    atopic.authorname = article.username;
    atopic.title = article.title;
    atopic.usericon = article.avatar;
    atopic.postsdate = article.dateline;
    atopic.subtract = article.summary;
    atopic.favid = article.favid;
    NSMutableArray * contentarray = [[NSMutableArray alloc] initWithCapacity:0];
    for (NSInteger index = 0; index < article.content.count; index ++) {
        article_content * art_cont = [article.content objectAtIndex:index];
        content * cont = [self article_contentToconent:art_cont ];
        [contentarray addObject:cont];
    }
    if(contentarray.count)
    {
        article_content * art_cont = [contentarray objectAtIndex:0];
        content * cont = [self article_contentToconent:art_cont];
        if (cont.msg.length > 80) {
            cont.msg = [NSString stringWithFormat:@"摘要: %@\n\n",[cont.msg substringToIndex:80]];
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
        if (indexPath.row == 0) {
            ...
            cell.celltopic = [self mainarticleTotopic:self.articleModel.mainArcticle]; //self.postmodel.maintopic;
            cell.lblfloortext = [NSString stringWithFormat:@"楼主: "];
            cell.support = nil; //self.articleModel.mainArcticle.s postmodel.maintopic.support;
...
//                cell.status = self.postmodel.maintopic.status;
            ...
            B3_PortalTableViewCell *cell =[tableView dequeueReusableCellWithIdentifier:indeifier];
            if (!cell) {
                cell = [[B3_PortalTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:indeifier];
                cell.delegate = self;
                cell.selectionStyle = UITableViewCellSelectionStyleNone;
            }
            cell.cellIndex = [NSString stringWithFormat:@"%d", indexPath.row - 1];
            commentlist * acomment = [self.articleModel.shots objectAtIndex:indexPath.row -1];
            cell.cellpost = [self commentlist2post:acomment];
            // [self.articleModel.shots objectAtIndex:indexPath.row - 1];
...
//    if ([self.articleType isEqualToNumber:@1]) {
        if (indexPath.row == 0) {
             ...
             */
            float height = [B3_PostTableView_HeadCell heightOfCell:[self articlecontent2contentAry:self.articleModel.mainArcticle.content]];
            height = height + 100 ;
            return height;
            //        return headercellheight;
        } else {
            post *apost =  [self commentlist2post:[self.articleModel.shots objectAtIndex:indexPath.row -1]];
            //[self.postmodel.shots objectAtIndex:indexPath.row - 1];
            float height = [B3_PortalTableViewCell heightOfCell:apost.content];
            height = height + 60 ;
            return height;
        }
//    }
...
//                return [PFTableViewCell heightAtIndexPath:indexPath];
//            } else {
//                commentlist *commentlist = self.articleModel.shots[indexPath.row - 1];
...
- (void)B3_CellReplyBtnTapped:(B3_PostTableView_Cell *)object
{
    ...
    if (self.friendpost) {
        self.articleReplyModel.cid = self.friendpost.pid.length?self.friendpost.pid:self.pid;// pid;// .maintopic.pid;
...
- (void)D2_ChatInputdShouldSendMessage:(UITextView *)textField
{
    ...
    self.articleReplyModel = [ArticleReplyModel modelWithObserver:self];
        ...
        if (self.friendpost)
        {
            self.articleReplyModel.aid = self.friendpost.pid.length?self.friendpost.pid:self.tid;
            NSString * usrname = [NSString stringWithFormat:@"回复%@中...",self.friendpost.authorname];
            [self presentLoadingTips:usrname];
            self.friendpost = nil;
//            replayField.placeholder = [NSString stringWithFormat:@"回复%@...", self.friendpost.authorname];
        }
        else
        {
            NSString * usrname = [NSString stringWithFormat:@"回复楼主中..."];
            [self presentLoadingTips:usrname];
        }
