...
- (void)setupTableViewCell:(PFTableViewCell *)cell inTableView:(UITableView *)tableView viewControl:(UIViewController *)viewControl indexPath:(NSIndexPath *)indexPath
{
    ...
    cell.thirdTextLabel.textAlignment = NSTextAlignmentRight;
    cell.thirdTextLabel.backgroundColor = [UIColor clearColor];
    cell.thirdTextLabel.font = [UIFont systemFontOfSize:14];
    cell.thirdTextLabel.frame = CGRectMake(cell.firstImageView.frame.size.width, indexPath.row == 0 ? cell.bounds.origin.y + 55 : cell.bounds.origin.y + 10, 60, 30);
    cell.fourthTextLabelShow = YES;
    cell.fourthTextLabel.font = [UIFont systemFontOfSize:14];
    cell.fourthTextLabel.frame = CGRectMake(cell.thirdTextLabel.frame.origin.x + cell.thirdTextLabel.frame.size.width, cell.thirdTextLabel.frame.origin.y, 200, cell.thirdTextLabel.frame.size.height);
    ...
    if (indexPath.row == 0) {
        ...
        //分割线
        UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, cell.bounds.origin.y + 100, cell.bounds.size.width, 0.5f)];
...
- (void)setupRow0InTableViewCell:(PFTableViewCell *)cell articleModel:(ArticleModel *)articleModel viewControl:(UIViewController *)viewControl indexPath:(NSIndexPath *)indexPath
{
    ...
    cell.thirdTextLabel.text = @"发表者:";
    cell.fourthTextLabel.text = articleModel.mainArcticle.username;
...
//    [postView loadContents:commentlist1.message height:[postView heightOfContents:commentlist1.message cellType:cellTypeArticla otherViewHeight:cell.bounds.origin.y + 50 indexPath:indexPath] contentView:cell.firstContentView];
    ...
    cell.thirdTextLabel.text = [NSString stringWithFormat:@"%@楼:", commentlist1.position];
    cell.fourthTextLabel.text = commentlist1.username;
