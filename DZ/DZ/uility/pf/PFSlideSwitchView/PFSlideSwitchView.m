...
typedef NSUInteger (^numberOfItemBlock)(PFSlideSwitchView *);
typedef UIViewController *(^viewControllerOfItemBlock)(PFSlideSwitchView *, NSUInteger);
typedef CGSize (^textSizeOfItemBlock)(PFSlideSwitchView *);
typedef void (^slideToEdgeBlock)(PFSlideSwitchView *, UIPanGestureRecognizer *);
typedef void (^didSelectItemBlock)(PFSlideSwitchView *, NSUInteger);

@interface PFSlideSwitchView () <UIScrollViewDelegate>
{
...
@implementation PFSlideSwitchView
...
- (void)loadSubviews
{
    ...
    else number = [self.delegate numberOfItemInSlideSwitchView:self];
...
//标签的个数
- (void)numberOfItemInSlideSwitchViewUsingBlock:(NSUInteger (^)(PFSlideSwitchView *))block
{
    if (block) self.numberOfItemBlock = block;
}

//设置视图控制器
- (void)viewControllerOfItemAtIndexUsingBlock:(UIViewController *(^)(PFSlideSwitchView *, NSUInteger))block
{
    if (block) self.viewControllerOfItemBlock = block;
}

//调整文本尺寸
- (void)textSizeOfItemInSlideSwitchViewUsingBlock:(CGSize (^)(PFSlideSwitchView *))block
{
    if (block) self.textSizeOfItemBlock = block;
}

//滑动到左边缘事件
- (void)slideToLeftEdgeUsingBlock:(void (^)(PFSlideSwitchView *, UIPanGestureRecognizer *))block
{
    if (block) self.slideToLeftEdgeBlock = block;
}

//滑动到右边缘事件
- (void)slideToRightEdgeUsingBlock:(void (^)(PFSlideSwitchView *, UIPanGestureRecognizer *))block
{
    if (block) self.slideToRightEdgeBlock = block;
}

//点击标签事件
- (void)didSelectItemAtIndexUsingBlock:(void (^)(PFSlideSwitchView *, NSUInteger))block
{
    ...
    if (!button.selected) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            if (finished) {
                ...
                } else if ([self.delegate respondsToSelector:@selector(slideSwitchView:didSelectItemAtIndex:)]) {//监听代理并回调
                    [self.delegate slideSwitchView:self didSelectItemAtIndex:selectedItem - 100];
                }
            }
        }];
    }
...
-(void)slideToEdge:(UIPanGestureRecognizer *)recognizer
{
    ...
    if (rootScrollView.contentOffset.x <= 0) {
        ...
        } else if ([self.delegate respondsToSelector:@selector(slideSwitchView:slideToLeftEdge:)]) {//监听代理并回调
            [self.delegate slideSwitchView:self slideToLeftEdge:recognizer];
        }
    }
    ...
    //当滑道右边缘时
        ...
        } else if ([self.delegate respondsToSelector:@selector(slideSwitchView:slideToRightEdge:)]) {//监听代理并回调
            [self.delegate slideSwitchView:self slideToRightEdge:recognizer];
        }
    }
}
