...
@interface D2_Chat_StrangerViewController ()<UITextFieldDelegate,MaskViewDelegate,D2_ChatInputViewDelegate,DZ_TimerDelegate,D2_ChatMessageCellDelegate>
{
...
@implementation D2_Chat_StrangerViewController
...
-(void)loadData
{
    ...
    NSArray *array= nil;
     if(self.chattype == CHATTYPE_STRANGER)
    {
        if (!self.allpmmodel.strangermsDic.count) {
            return;
        }
        array=[self.allpmmodel.strangermsDic objectForKey:self.afriend.fuid];
        if (!array.count) {
            return;
        }
        ...
        for (strangerms *dict in array) {
            ...
            afriend.message = [self newText:afriend.message];
...
//去掉****对你说: ****前面的前缀
-(NSString *)newText:(NSString *)message
{
