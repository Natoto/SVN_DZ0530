...
- (void)routine
{
	if ( self.sending )
	{
        ...
        self.HTTP_POST(requestURI).PARAM([self.req objectToDictionary]).HEADER(@"ContentType", @"application/x-www-form-urlencoded; charset=utf-8").PARAM(@"content",contentbase64).PARAM(@"fid",self.fid).PARAM(@"authorid",self.authorid).PARAM(@"tid",self.tid).PARAM(@"pid",self.pid).PARAM(@"client",[ServerConfig sharedInstance].client);
