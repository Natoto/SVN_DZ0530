...
@property (nonatomic, retain) ForumlistModel * fmModel;
@end
...
- (void)unload
{
    [self.fmModel removeObserver:self];
    self.fmModel = nil;
}
...
{
    BeeLog(@"=======ForumlistModel.RELOADED =======");
    [self FinishedLoadData];
    [self.list reloadData];
}
...
//    [self.fmModel firstPage];
    @weakify(self);
    [ForumlistModel getForumlist:^(NSArray *reciveData) {
        @normalize(self); 
        self.fmModel.shots =[NSMutableArray arrayWithArray:reciveData];
        [self FinishedLoadData];
        [self.list reloadData];
        BeeLog(@"----------b0_formplates 加载成功----------");
    } errorblock:^(NSString *errmsg) {
        [self presentMessageTips:errmsg];
        [self FinishedLoadData];
        [self.list reloadData];
         BeeLog(@"----------b0_formplates 加载失败-----------");
    }];
}
