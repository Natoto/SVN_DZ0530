...
- (id)initWithFrame:(CGRect)frame itemNumber:(NSInteger)itemNumber
{
    self = [super initWithFrame:frame];
    if (self) {
        // Initialization code
...
-(IBAction)BTNACTIONS:(id)sender
{
    ...
    if (self.itemNumber == 3) {
        for (int index = 0; index < 3; index++) {
            UIButton *button=(UIButton *)[_backGroundView viewWithTag:index + 3 + ITEMSSTARTTAG];
...
{ 
   if (!_backGroundView) {
       ...
       NSArray *array;
       if (self.itemNumber == 3) {
           items=[NSDictionary dictionaryWithObjectsAndKeys:@"3",@"分享",@"4",@"收藏",@"5",@"复制链接", nil];
           array = [[NSArray alloc] initWithObjects:@"分享",@"收藏", @"复制链接", nil];
       } else {
           items=[NSDictionary dictionaryWithObjectsAndKeys:@"0",@"只看楼主",@"1",@"倒序看帖",@"2",@"回复",@"3",@"分享",@"4",@"收藏",@"5",@"复制链接", nil];
           //,@"5",@"取消收藏"
           array = [[NSArray alloc] initWithObjects:@"只看楼主",@"倒序看帖",@"回复",@"分享",@"收藏", @"复制链接", nil];
       }
//       items=[NSDictionary dictionaryWithObjectsAndKeys:@"0",@"只看楼主",@"1",@"倒序看帖",@"2",@"回复",@"3",@"分享",@"4",@"收藏",@"5",@"复制链接", nil];
       //,@"5",@"取消收藏"
