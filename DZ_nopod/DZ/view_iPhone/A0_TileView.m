...
//        self.imgView.data=item.icon;
        //localhost:tianjia@2x.jpg
        if ([item.icon rangeOfString:@"localhost:"].location !=NSNotFound) {
            NSArray *array = [item.icon componentsSeparatedByString:@":"];
            if (array.count ==2) {
                self.imgView.image =[UIImage bundleImageNamed:[array objectAtIndex:1]];
                self.imgView.command = COMMAND_NOPERMISSION;
            }
        }
        else
