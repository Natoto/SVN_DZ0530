...
-(void)reloadData
{
    if (self.homeModel) {
        ...
        if (self.homeModel.onoff.iscommand.integerValue) {
            HOME2TOPICSPOSITIONITEM *itemh4 = [[HOME2TOPICSPOSITIONITEM alloc] init];
            home_command *acommand = self.homeModel.shots.command;
            itemh4.title = acommand.title ? acommand.title : @"站长推荐";
            itemh4.subject = acommand.subject ? acommand.subject : @"这个站长好懒，没有推荐什么";
            itemh4.icon = acommand.img;
            itemh4.fid = HOME_FID_TUIJIAN;
            itemh4.count = @"0";
            itemh4.enableDelete = [NSString stringWithFormat:@"0"];
            [tempModeleBlocks insertObject:itemh4 atIndex:0];
        }
...
//                break;
    ...
    {
        NSInteger index = 0;
        HOME2TOPICSPOSITIONITEM *item = blocks[1];
