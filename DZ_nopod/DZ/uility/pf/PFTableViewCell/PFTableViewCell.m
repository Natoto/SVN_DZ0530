...
typedef void(^eventsBlock)(NSIndexPath *);
typedef void(^controlTouchBlock)(NSIndexPath *, NSInteger);
...
///加载完成
@property (nonatomic, copy) eventsBlock                 loadedBlock;

///被选中
@property (nonatomic, copy) eventsBlock                 selectedBlock;
...
- (void)layoutSubviews
{
    ...
    //加载完成
    if ([self.delegate respondsToSelector:@selector(tableViewCell:loadedAtIndexPath:)]) {//监听代理并回调
        [self.delegate tableViewCell:self loadedAtIndexPath:self.indexPath];
...
//被选中
- (void)selectedUsingBlock:(void (^)(NSIndexPath *))block
{
    if (block) self.selectedBlock = block;
}

//按钮点击
- (void)buttonTouchUsingBlock:(void (^)(NSIndexPath *, NSInteger))block
{
    if (block) self.buttonBlock = block;
}

//图片点击
- (void)imageViewTouchUsingBlock:(void (^)(NSIndexPath *, NSInteger))block
{
    if (block) self.imageViewBlock = block;
}
...
- (void)setSelected:(BOOL)selected animated:(BOOL)animated
{
    ...
    if ([self.delegate respondsToSelector:@selector(tableViewCell:selectedAtIndexPath:)]) {//监听代理并回调
        [self.delegate tableViewCell:self selectedAtIndexPath:self.indexPath];
    } else if (self.selectedBlock) {//监听块并回调
...
- (void)buttonTouch:(UIControl *)control
{
    if ([self.delegate respondsToSelector:@selector(tableViewCell:buttonTouchAtIndexPath:controlIndex:)]) {//监听代理并回调
        ...
        self.buttonBlock(self.indexPath, control.tag);
    }
}
...
- (void)imageViewTouch:(UITapGestureRecognizer *)tapGestureRecognizer
{
    if ([self.delegate respondsToSelector:@selector(tableViewCell:imageViewTouchAtIndexPath:controlIndex:)]) {//监听代理并回调
        ...
        self.imageViewBlock(self.indexPath, tapGestureRecognizer.view.tag);
    }
}
